...
t() {
    ...
    case "$(normalize_locale "$AGENT_LOCALE"):$key" in
        ...
        eu_ES:invalid_config) printf '%s' "config.env-ek ez ditu baliozko token, UUID eta alias baliorik." ;;
        ...
        gl_ES:invalid_config) printf '%s' "config.env non contén valores de token, UUID e alias válidos." ;;
        ...
        fr_FR:invalid_config) printf '%s' "config.env ne contient pas de valeurs de jeton, d'UUID et d'alias valides." ;;
        ...
        de_DE:invalid_config) printf '%s' "config.env enthält keine gültigen Token-, UUID- und Aliaswerte." ;;
        ...
        it_IT:invalid_config) printf '%s' "config.env non contiene valori token, UUID e alias validi." ;;
        ...
        ja_JP:invalid_config) printf '%s' "config.env には、有効なトークン、UUID、およびエイリアスの値が含まれていません。" ;;
        ...
        zh_CN:invalid_config) printf '%s' "config.env 不包含有效的令牌、UUID 和别名值。" ;;
